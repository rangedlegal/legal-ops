...
export default function EditClientPage() {
      ...
      if (client && !client.error) {
        setForm({
          ...
          billingStartDate: client.billingStartDate
            ? new Date(client.billingStartDate).toISOString().split("T")[0]
            : "",
          accountOwnerId: client.accountOwnerId ?? "",
          status: client.status ?? "ACTIVE",
          includedScope: client.includedScope ?? "",
          excludedScope: client.excludedScope ?? "",
          monthlyEffortAllowance: client.monthlyEffortAllowance != null
            ? String(client.monthlyEffortAllowance)
            : "",
          notes: client.notes ?? "",
        })
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setError(data?.error?.formErrors?.join(", ") || "Failed to update client.")
      setSubmitting(false)
    }
  }
    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Update client details</p>
        </div>
      </div>
